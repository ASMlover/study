...
test("navigation then Tab accepts selected candidate command", async (t) => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "minicli-nav-complete-"));
  ...
  try {
    if (!blocked) {
      ...
      assert.match(stderr, /Usage: \/switch <#id\|index>/);
