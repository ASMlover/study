...
test("/mo+Tab accepts completion to /model", async (t) => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "minicli-tab-complete-"));
  ...
  try {
    if (!blocked) {
      ...
      assert.equal(stderr, "");
      assert.match(stdout, /Current model: mock-mini/);
      assert.doesNotMatch(stdout, /Completions:/);
    }
  } finally {
    fs.rmSync(tmpRoot, { recursive: true, force: true });
  }
