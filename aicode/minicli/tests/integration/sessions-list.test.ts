...
test("/sessions lists multi-session output in recent order", async (t) => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "minicli-sessions-list-"));
  ...
  try {
    ...
    assert.equal(stderr, "");
    assert.match(stdout, /\* \[1\] #2 beta/);
    assert.match(stdout, /  \[2\] #1 alpha/);
  } finally {
    fs.rmSync(tmpRoot, { recursive: true, force: true });
  }
});
