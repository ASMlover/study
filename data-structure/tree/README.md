...
RB删除节点修正的几种情况：
 * case1：x的兄弟节点w是红色
   - 因为w必须是黑色子节点，所以要改变w和x.p的颜色，然后对x.p做一次左旋；
   - 这样x的新兄弟节点是旋转之前w的某个（颜色为黑色）子节点，这样就转变为情况2、3、4
 * case2：x的兄弟节点w是黑色的，而且w的两字节点都是黑色的
   - w的两个子节点都是黑色的，因为w也是黑色的，所以从x和w上去掉一重黑色使得x只有一重黑色而w为红色，在原来是红色或黑色的x.p上新增一重额外的黑色；将x.p作为新的x节点来重复while循环
 * case3：x的兄弟节点w是黑色的，w的左孩子是红色的，w的右孩子是黑色的
   - 交换w和其左孩子w.left的颜色，对w进行右旋
   - x的新兄弟节点w是一个有红色右孩子的黑色节点，这样转换为情况4
 * case4：x的兄弟节点w是黑色的，且w的右孩子是红色的
   - 将某些颜色修改并对x.p做一次左旋，则去掉x的额外黑色使它变成单重黑色
   - 将x设置为根后，当while循环测试其循环条件时，循环终止
